...
気になる人は最初の返信チェックしてみて！`,
    ...
    // 3단계: 첫 댓글 고정용 수익화 문구 (링크 3회 반복 탑재!)
    const pinned_comment_ko = `${ftcDisclaimer}

다들 이거 쓸 때 제일 빡치는 게 뭐야? 😂

✓ ${name} 하나로 시간 단축 & 설거지 1개로 끝
✓ 손목에 무리 없이 누구나 1초 컷 완성
...
${link}`;
    ...
    const pinned_comment_ja = `みんながこれ使う時、一番イライラすることって何？ ✨

✓ ${name} で時短＆洗い物から完全に解放
✓ 手首に負担なく誰でも1秒でプロ並みの仕上がり

🔽 動画で使った愛用アイテムはこちら 🔽
${isAmazon ? link : 'https://amzn.to/example'}
${isAmazon ? link : 'https://amzn.to/example'}
...
#${name.replace(/\s+/g, '')} #살림템추천 #내돈내산추천 #삶의질향상 #인생템 #자취꿀템 #주부스타그램 #꿀템리뷰 #살림노하우 #생활꿀팁 #쿠팡추천템 #가성비갑 #살림스타그램 #신박템 #홈스타그램`;
  ...
  generateCoupangAutoDmKit(p) {
    ...
    return {
      ...
      followerDm: `안녕하세요! 요청하신 [${name}] 최저가 구매 정보입니다.
늘 응원해 주셔서 감사합니다 ❤️ 아래 상품링크 버튼을 눌러 확인해 보세요!`,
      nonFollowerDm: `요청하신 [${name}] 최저가 구매 정보입니다! 아래 상품링크 버튼을 확인해 보세요.
